...
@TeleOp
public class TELEOP_MAIN_TEST extends LinearOpMode {
    ...
    public double aimHeading;
    public static final String X_POSITION_KEY = "X Position";
    public static final String Y_POSITION_KEY = "Y Position";
    public static final String HEADING_KEY = "Heading";
    public static final String ALLIANCE_KEY = "Alliance";
    //public Object colorAlliance = blackboard.get(ALLIANCE_KEY);
    public String colorAlliance = "BLUE"; // for testing only, remove for final
    //double d = ((Number) obj).doubleValue();
    public double posStartX = ((Number) Objects.requireNonNull(blackboard.get(X_POSITION_KEY))).doubleValue();  //check these, might throw a null pointer exception
    public double posStartY = ((Number) Objects.requireNonNull(blackboard.get(Y_POSITION_KEY))).doubleValue();
    public double posStartH = ((Number) Objects.requireNonNull(blackboard.get(HEADING_KEY))).doubleValue();
    ...
    public void runOpMode() {
        ...
        if (opModeIsActive()) {
            while (opModeIsActive()) {
                ...
                telemetry.addData("Aim Heading", aimHeading);
   ...
   /* private void farPowerAuto() {
    ...
    /**
     * This function uses Math.atan2() to find the double arctangent based on the bot position to the corners of the field.
     * atan2() is a common method to find the angle between two points in a plane.  We learned about it and used it in 2024-2025 Into the Deep as well.
     * https://en.wikipedia.org/wiki/Atan2
     */
    private void getAimHeading(){
        SparkFunOTOS.Pose2D pos = poseOTOS.getPosition();
        if (colorAlliance=="BLUE"){
            aimHeading = Math.atan2(-71-pos.y,-71-pos.x);
        }
        else{
            aimHeading = Math.atan2(71-pos.y,-71-pos.x);
        }
    ...
    /**
     * This function used Roadrunner to rotate the bot to aim at the correct alliance goal.
     * Roadrunner is a blocking function, so pressing this button will result in the function running to completion before doing anything else.
     */
    private void aimBot(){
        if(gamepad2.left_trigger>0.1){
            SparkFunOTOS.Pose2D pos = poseOTOS.getPosition();
            Pose2d aimBeginPose = new Pose2d(pos.x, pos.y, pos.h);
            //Instantiate the roadrunner Mecanum drive (via the OTOS localizer)
            //SparkFunOTOSDrive drive = new SparkFunOTOSDrive(hardwareMap, beginPose);
            MecanumDrive drive = new MecanumDrive(hardwareMap, aimBeginPose);
            drive.localizer.setPose(aimBeginPose);  //may have to do this for the new RR version per https://community.sparkfun.com/t/sparkfun-otos-with-ftc-inital-pose-always-0/67256

            getAimHeading();
            //Build the actions for our AUTO mode
            Actions.runBlocking(
                    drive.actionBuilder(aimBeginPose)
                            .turnTo(aimHeading)
                            .build());
        }
    ...
    private void configureOtos() {
        ...
        SparkFunOTOS.Pose2D currentPosition = new SparkFunOTOS.Pose2D(posStartX, posStartY, posStartH);
