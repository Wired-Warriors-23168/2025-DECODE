...
@TeleOp
public class TELEOP_MAIN_PS_CONTROLS extends LinearOpMode {
    ...
    private void aimBot() {
        ...
        switch (shooterState) {
            ...
            case WAITING_FOR_SERVO:
                if (servoTime.milliseconds() - startServoTime > 100) {
...
//        }
        ...
        } else if (gamepad2.dpadRightWasPressed() && ballnumber > 0) {             // Forced shooting: Purple
            purpleServo.setPosition(purpleShootPos);
            sleep(250);
            purpleServo.setPosition(purpleDownPos);
            ++ballnumber;
        } else if (gamepad2.dpadLeftWasPressed() && ballnumber > 0) {      // Forced shooting: Green
